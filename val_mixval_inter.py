...
def test(config, args, net):
    ...
    dsets["test_tgt"] = ImageList(open(data_config["test_tgt"]["list_path"]).readlines(), transform=prep_dict["test_tgt"])
...
def mixval(net, args, config):
    ...
    score_ours = [target_acc, ice_inter]
    ...
    log_str = "Ep:{}, synth:{}, lam:{:.4f}, score:{}, tgtAcc and ice_inter:{}, numMix:{}.\n".\
...
if __name__ == "__main__":
    ...
    config["output_path"] = os.path.join('./logs/inter/'+args.da, args.da+'_'+str(args.epoch)+'ep_run'+str(args.run)+'_'+args.synth+\
    ...
    config["total_log_file"] = open(osp.join('./logs/inter/'+args.da, args.da+'_'+str(args.epoch)+'ep_run'+str(args.run)+'_'+args.synth+\
